...
{
    private readonly IProductRepository _productRepository; private readonly ICategoryRepository _categoryRepository; private readonly IUnitOfWork _unitOfWork;
    ...
    public async Task<PagedResponse<ProductListItemResponse>> SearchAsync(ProductQueryRequest q,CancellationToken ct=default){var r=await _productRepository.SearchAsync(new ProductQuery{SearchTerm=q.SearchTerm,CategoryId=q.CategoryId,Gender=q.Gender,IsActive=q.IsActive,IsFeatured=q.IsFeatured,SortBy=q.SortBy,Page=q.Page<=0?1:q.Page,PageSize=q.PageSize is <=0 or >100?12:q.PageSize},ct);return new(){Items=r.Items.Select(MapList).ToList(),TotalCount=r.TotalCount,Page=r.Page,PageSize=r.PageSize,TotalPages=r.TotalPages};}
    ...
    public Task ActivateAsync(Guid id,CancellationToken ct=default)=>ChangeActive(id,true,ct); public Task DeactivateAsync(Guid id,CancellationToken ct=default)=>ChangeActive(id,false,ct);
